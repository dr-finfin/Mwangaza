...
export default function VideoPlayer({ video, subTopic, topicName, onBack }: VideoPlayerProps) {
    ...
    const saveProgress = async () => {
      try {
        ...
        await fetch('/api/progress', {
          ...
          body: JSON.stringify({
            lesson_id: video.videoId, // Using videoId as lesson_id
            lesson_name: video.title,
            completed: true, // Marking as completed when they open the video for now
          }),
        });
      } catch (err) {
  ...
  return (
    <div className="p-8">
      ...
      <h1 className="text-3xl font-bold text-blue-900 dark:text-blue-300 mb-2">{video.title}</h1>
