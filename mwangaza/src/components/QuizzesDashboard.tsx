...
export default function QuizzesDashboard() {
  ...
  const handleStartTopicTest = async (topic: Topic) => {
    ...
    const questions = await generateQuiz(topic.name);
  ...
  const handleStartSubTopicQuiz = async (subTopic: SubTopic) => {
    ...
    const questions = await generateQuiz(selectedTopic!.name, subTopic.name);
    setQuizQuestions(questions);
    setIsLoading(false);
  };
  ...
  const handleStartFlashcards = async (subTopic: SubTopic) => {
    ...
    const cards = await generateFlashcards(selectedTopic!.name, subTopic.name);
    setFlashcards(cards);
    setIsLoading(false);
  };
  ...
  const renderQuiz = () => {
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center py-20">
          <div className="w-12 h-12 border-4 border-blue-600 border-solid rounded-full border-t-transparent animate-spin mb-4"></div>
          <p className="text-xl text-blue-800 font-medium">Generating your quiz...</p>
        </div>
      );
    }
    ...
    if (quizQuestions.length === 0) {
      return (
        <div className="text-center py-20">
          <p className="text-xl text-red-600">Failed to load quiz. Please try again.</p>
          <button onClick={() => setViewMode('selection')} className="mt-4 text-blue-600 hover:underline">Go Back</button>
        </div>
      );
    }
    ...
    if (quizCompleted) {
      return (
        <div className="bg-white p-8 rounded-xl shadow-lg text-center max-w-2xl mx-auto">
          ...
          <p className="text-2xl text-slate-700 mb-8">You scored <span className="font-bold text-blue-600">{score}</span> out of {quizQuestions.length}</p>
          <button 
    ...
    return (
      <div className="max-w-3xl mx-auto">
        <button onClick={() => setViewMode('selection')} className="flex items-center text-blue-800 hover:text-yellow-500 mb-6 font-bold">
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-slate-500">Question {currentQuestionIndex + 1} of {quizQuestions.length}</h2>
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full font-bold">Score: {score}</span>
          </div>
          
          <h3 className="text-2xl font-bold text-blue-900 mb-8">{question.question}</h3>
          ...
          {showExplanation && (
            <div className={`p-4 rounded-lg mb-6 ${selectedAnswer === question.correctAnswerIndex ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
  ...
  const renderFlashcards = () => {
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center py-20">
          <div className="w-12 h-12 border-4 border-blue-600 border-solid rounded-full border-t-transparent animate-spin mb-4"></div>
          <p className="text-xl text-blue-800 font-medium">Generating flashcards...</p>
        </div>
      );
    }
    ...
    if (flashcards.length === 0) {
      return (
        <div className="text-center py-20">
          <p className="text-xl text-red-600">Failed to load flashcards. Please try again.</p>
          <button onClick={() => setViewMode('selection')} className="mt-4 text-blue-600 hover:underline">Go Back</button>
        </div>
      );
    }
    ...
    return (
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-blue-900">{selectedSubTopic?.name} Flashcards</h2>
          <span className="text-slate-500 font-bold">{currentFlashcardIndex + 1} / {flashcards.length}</span>
        </div>
        ...
        >
          <div className={`w-full h-full transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
            {/* Front */}
            <div className="absolute w-full h-full backface-hidden bg-white rounded-2xl shadow-xl border-4 border-blue-100 flex flex-col items-center justify-center p-8 text-center">
              <span className="absolute top-4 left-4 text-blue-400 font-bold uppercase tracking-wider text-sm">Front</span>
              <h3 className="text-3xl font-bold text-blue-900">{card.front}</h3>
              <p className="absolute bottom-4 text-slate-400 text-sm">Click to flip</p>
            </div>
            {/* Back */}
            <div className="absolute w-full h-full backface-hidden bg-blue-50 rounded-2xl shadow-xl border-4 border-blue-200 flex flex-col items-center justify-center p-8 text-center rotate-y-180">
              <span className="absolute top-4 left-4 text-blue-500 font-bold uppercase tracking-wider text-sm">Back</span>
              <p className="text-2xl text-blue-900 leading-relaxed">{card.back}</p>
        ...
        <div className="flex justify-between mt-8">
          <button 
            ...
            className="px-6 py-3 bg-white text-blue-600 font-bold rounded-lg shadow disabled:opacity-50"
          >
            Previous
          </button>
          <button 
            ...
            className="px-6 py-3 bg-blue-600 text-white font-bold rounded-lg shadow hover:bg-blue-700 disabled:opacity-50"
          >
            Next
          </button>
        </div>
  ...
  const renderSelection = () => {
    if (selectedTopic) {
      return (
        <div>
          <button onClick={() => setSelectedTopic(null)} className="flex items-center text-blue-800 hover:text-yellow-500 mb-6 font-bold">
            <ArrowLeft className="mr-2" />
            Back to Topics
          </button>
          
          <div className="flex justify-between items-center mb-8">
            <h1 className="text-3xl font-bold text-blue-900">{selectedTopic.name}</h1>
            <button 
              onClick={() => handleStartTopicTest(selectedTopic)}
              className="bg-yellow-400 text-blue-900 font-bold py-3 px-6 rounded-lg flex items-center hover:bg-yellow-500 transition shadow-md"
            >
          ...
          <h2 className="text-2xl font-bold text-blue-800 mb-4">Sub-Topics</h2>
          <div className="space-y-4">
            {selectedTopic.subTopics.map((subTopic) => (
              <div key={subTopic.name} className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex justify-between items-center">
                <div>
                  <h3 className="text-xl font-bold text-blue-900">{subTopic.name}</h3>
                  <p className="text-slate-500 mt-1">Test your knowledge or study with flashcards.</p>
                </div>
                <div className="flex gap-3">
                  <button 
                    onClick={() => handleStartFlashcards(subTopic)}
                    className="px-4 py-2 bg-blue-50 text-blue-700 font-bold rounded-lg hover:bg-blue-100 transition flex items-center"
                  >
                  ...
                  <button 
                    ...
                    className="px-4 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition flex items-center"
                  >
