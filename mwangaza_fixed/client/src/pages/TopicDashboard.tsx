...
import DashboardLinkCard from '../components/ui/DashboardLinkCard';
...
export default function TopicDashboard() {
  ...
  const handleSubTopicSelect = (subTopic: SubTopic, topicName: string) => {
    // In a real app, this would navigate to a specific sub-topic page
    // For now, we'll just log it and navigate to the generic topic page
    console.log('Selected SubTopic:', subTopic, 'from Topic:', topicName);
    navigate('/topics', { state: { subTopic, topicName } });
  };
  ...
  const renderContent = () => {
    ...
    if (selectedTopic) {
      return (
        <div>
          ...
          <h1 className="text-3xl font-bold text-blue-900 dark:text-gray-100 mb-6">{selectedTopic.name} - Sub-Topics</h1>
          <div className="space-y-3">
            {selectedTopic.subTopics.map(subTopic => (
              <div key={subTopic.name}
                className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm cursor-pointer hover:bg-yellow-100 dark:hover:bg-yellow-900/20 transition"
                onClick={() => handleSubTopicSelect(subTopic, selectedTopic.name)}>
    ...
    if (selectedGradeContent) {
      return (
        <div>
          ...
          <h1 className="text-3xl font-bold text-blue-900 dark:text-gray-100 mb-6">{selectedCategory!.name} - {selectedGradeContent.grade}</h1>
    ...
    if (selectedCategory) {
      return (
        <div>
          ...
          <h1 className="text-3xl font-bold text-blue-900 dark:text-gray-100 mb-6">{selectedCategory.name} - Select a Grade</h1>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {selectedCategory.contentByGrade.map((content) => (
              <div key={content.grade}
                className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg cursor-pointer hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col items-center justify-center border-4 border-transparent hover:border-yellow-400 dark:hover:border-yellow-500"
    ...
    return (
      <>
        <h1 className="text-3xl font-bold text-blue-900 dark:text-gray-100 mb-8">Jambo! Welcome to Mwangaza.</h1>
